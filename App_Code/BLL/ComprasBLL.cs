...
public class ComprasBLL
{
    ...
    public void COM_GASTOS_EXT_DET_DELETE(long COM_GASTOS_EXT_CAB_ID)
    {
        try
        {
            ComprasDALTableAdapters.QueriesTableAdapter adapter = new ComprasDALTableAdapters.QueriesTableAdapter();
            adapter.H2_COM_GASTOS_EXT_DET_DELETE(COM_GASTOS_EXT_CAB_ID);
        }
        catch (SqlException ex)
        {
            throw new Exception(ex.Message);
    ...
    public int Asiganar_Insumo_Alto_Costo(long id)
    {
        try
        {
            ComprasDALTableAdapters.QueriesTableAdapter adapter = new ComprasDALTableAdapters.QueriesTableAdapter();
            object obj = adapter.H2_Asiganar_Insumo_Alto_Costo(id);
    ...
    public List<compras_insumos_combo> List_InsumosCombo(bool Todos)
    {
        ...
        try
        {
            ...
            foreach (ComprasDAL.H2_COMPRAS_LIST_INSUMOS_COMBORow row in aTable.Rows)
                list.Add(new compras_insumos_combo(row.INS_ID, row.INS_DESCRIPCION.Trim(),row.INS_RUBRO,row.INS_ALTO_COSTO));
            return list;
        }
        catch (SqlException ex)
        {
    ...
    public List<com_ins_gastos_ext> COMPRAS_INSUMOS_INTERNACION_EXTRAORD(bool Todos)
    {
        try
        {
            List<com_ins_gastos_ext> list = new List<com_ins_gastos_ext>();
            ComprasDALTableAdapters.H2_COMPRAS_INSUMOS_INTERNACION_EXTRAORDTableAdapter adapter = new ComprasDALTableAdapters.H2_COMPRAS_INSUMOS_INTERNACION_EXTRAORDTableAdapter();
            ComprasDAL.H2_COMPRAS_INSUMOS_INTERNACION_EXTRAORDDataTable aTable = adapter.GetData(Todos);
            foreach (ComprasDAL.H2_COMPRAS_INSUMOS_INTERNACION_EXTRAORDRow row in aTable.Rows)
                list.Add(new com_ins_gastos_ext { COM_INS_GASTOS_EXT_ID = row.COM_INS_GASTOS_EXT_ID, COM_INS_GATOS_EXT_DESC = row.COM_INS_GATOS_EXT_DESC, COM_INS_GATOS_EXT_BAJA = row.COM_INS_GATOS_EXT_BAJA });
            return list;
        }
        catch (SqlException ex)
        {
        ...
        long NroPedidoId)
    {
        try
        {
            ...
            foreach (ComprasDAL.H2_EXPEDIENTES_BUSCARRow row in aTable.Rows)
            {
                ...
                string EXP_FEC_NAC = string.Empty;
                string foto;
                if(!row.IsfotoNEWNull()){ foto = row.fotoNEW; } else { foto = "0"; }

                if (!row.IsEXP_FEC_NACNull()) EXP_FEC_NAC = row.EXP_FEC_NAC.ToShortDateString();
                list.Add(new expediente_buscar(row.EXP_ID, (int)row.EXP_NRO_DOC, row.EXP_NOMBRE, row.Seccional, Fecha_venc, row.Patologias, row.Estado, EXP_FEC_NAC,row.EXP_OBS,foto));
            }
            return list;
        }
        catch (SqlException ex)
    ...
    public void COMPRAS_EXP_PEDIDOS_CAB_BAJA(long NroPedidoCAB, long UsuarioId)
    {
        try
        {
            ComprasDALTableAdapters.QueriesTableAdapter adapter = new ComprasDALTableAdapters.QueriesTableAdapter();
            adapter.H2_COMPRAS_EXP_PEDIDOS_CAB_BAJA(NroPedidoCAB, UsuarioId);
        }
        catch (SqlException ex)
        {
            throw new Exception(ex.Message);
    ...
    public com_gastos_ext_cab COM_GASTOS_EXT_CAB_LIST_BY_ID(long COM_GASTOS_EXT_CAB_ID)
    {
        try
        {
            ComprasDALTableAdapters.H2_COM_GASTOS_EXT_CAB_LIST_BY_IDTableAdapter adapter = new ComprasDALTableAdapters.H2_COM_GASTOS_EXT_CAB_LIST_BY_IDTableAdapter();
            ComprasDAL.H2_COM_GASTOS_EXT_CAB_LIST_BY_IDDataTable aTable = adapter.GetData(COM_GASTOS_EXT_CAB_ID);
            foreach (ComprasDAL.H2_COM_GASTOS_EXT_CAB_LIST_BY_IDRow row in aTable.Rows)
                return new com_gastos_ext_cab { COM_GASTOS_EXT_CAB_ID = row.COM_GASTOS_EXT_CAB_ID, COM_GASTOS_EXT_CAB_PACIENTE_ID = row.COM_GASTOS_EXT_CAB_PACIENTE_ID,
                     COM_GASTOS_EXT_CAB_MED_SOL = row.COM_GASTOS_EXT_CAB_MED_SOL, COM_GASTOS_EXT_CAB_SERV_ID = row.COM_GASTOS_EXT_CAB_SERV_ID, COM_GASTOS_EXT_CAB_FECHA_AUT = row.COM_GASTOS_EXT_CAB_FECHA_AUT.ToShortDateString(),
                      COM_GASTOS_EXT_CAB_FECHA_CIR = row.COM_GASTOS_EXT_CAB_FECHA_CIR.ToShortDateString(), COM_GASTOS_EXT_CAB_FECHA_SOL = row.COM_GASTOS_EXT_CAB_FECHA_SOL.ToShortDateString(),
                       COM_GASTOS_EXT_CAB_USU_ID = 0 };
                return null;
        }
        catch (SqlException ex)
        {
    ...
    public List<com_gastos_ext_det> COM_GASTOS_EXT_DET_LIST_BY_ID(long COM_GASTOS_EXT_CAB_ID)
    {
        try
        {
            List<com_gastos_ext_det> list = new List<com_gastos_ext_det>();
            ComprasDALTableAdapters.H2_COM_GASTOS_EXT_DET_LIST_BY_IDTableAdapter adapter = new ComprasDALTableAdapters.H2_COM_GASTOS_EXT_DET_LIST_BY_IDTableAdapter();
            ComprasDAL.H2_COM_GASTOS_EXT_DET_LIST_BY_IDDataTable aTable = adapter.GetData(COM_GASTOS_EXT_CAB_ID);
            foreach (ComprasDAL.H2_COM_GASTOS_EXT_DET_LIST_BY_IDRow row in aTable.Rows)
                list.Add(new com_gastos_ext_det
                {
                    COM_GASTOS_EXT_DET_CAB_ID = row.COM_GASTOS_EXT_DET_CAB_ID,
                    COM_GASTOS_EXT_DET_CANTIDAD = row.COM_GASTOS_EXT_DET_CANTIDAD,
                    COM_GASTOS_EXT_DET_INS_ID = row.COM_GASTOS_EXT_DET_INS_ID,
                    COM_GASTOS_EXT_DET_PRECIO_FACT = row.COM_GASTOS_EXT_DET_PRECIO_FACT,
                    COM_GASTOS_EXT_DET_PRECIO_PRESU = row.COM_GASTOS_EXT_DET_PRECIO_PRESU,
                    COM_GASTOS_EXT_DET_PRV_ID = row.COM_GASTOS_EXT_DET_PRV_ID,
                    COM_GASTOS_EXT_DET_USADO = row.COM_GASTOS_EXT_DET_USADO,
                    COM_INS_GASTOS_EXT_DESC = row.INS_NOMBRE,
                    COM_GASTOS_EXT_DET_PRV_NOMBRE = row.PROVEEDOR,
                    COM_GASTOS_EXT_DET_FECHA_FACT = (row.FECHA_FACT.ToShortDateString().Equals("01/01/1900")) ? "" : row.FECHA_FACT.ToShortDateString(),
                    COM_GASTOS_EXT_DET_NRO_FACTURA = row.NRO_FACTURA
                });
            return list;
        }
        catch (SqlException ex)
    ...
    public List<com_gastos_ext_listar> COM_GASTOS_EXT_CAB_BUSCAR(long COM_GASTOS_EXT_CAB_ID, string Paciente, string NHC, string Desde, string Hasta,
        int? Seccional, string Insumo)
    {
        try
        {
            List<com_gastos_ext_listar> list = new List<com_gastos_ext_listar>();
            ComprasDALTableAdapters.H2_COM_GASTOS_EXT_CAB_BUSCARTableAdapter adapter = new ComprasDALTableAdapters.H2_COM_GASTOS_EXT_CAB_BUSCARTableAdapter();
            ComprasDAL.H2_COM_GASTOS_EXT_CAB_BUSCARDataTable aTable = adapter.GetData(COM_GASTOS_EXT_CAB_ID, Paciente, NHC, DateTime.Parse(Desde),
                DateTime.Parse(Hasta),   Seccional, Insumo);
            foreach (ComprasDAL.H2_COM_GASTOS_EXT_CAB_BUSCARRow row in aTable.Rows)
                list.Add(new com_gastos_ext_listar
                {
                    COM_GASTOS_EXT_CAB_ID = row.COM_GASTOS_EXT_CAB_ID,
                    COM_GASTOS_EXT_CAB_FECHA_AUT = row.COM_GASTOS_EXT_CAB_FECHA_AUT.ToShortDateString(),
                    COM_GASTOS_EXT_CAB_FECHA_CIR = row.COM_GASTOS_EXT_CAB_FECHA_CIR.ToShortDateString(),
                    COM_GASTOS_EXT_CAB_FECHA_SOL = row.COM_GASTOS_EXT_CAB_FECHA_SOL.ToShortDateString(),
                    COM_GASTOS_EXT_DET_FECHA_FACT = row.fechaFactura.ToShortDateString(),
                    Servicio = row.Servicio,
                    Usuario = row.Usuario,
                    Paciente = row.Paciente,
                    NHC = row.NHC,
                    COM_GASTOS_EXT_DET_CANTIDAD = row.Cantidad,
                    COM_GASTOS_EXT_DET_PRECIO_FACT = row.Facturado,
                    COM_GASTOS_EXT_DET_PRECIO_PRESU = row.Presupuestado,
                    COM_INS_GASTOS_EXT_DESC = row.insumo,
                    Seccional = row.Seccional,
                    COM_GASTOS_OBSERVACIONES = row.COM_GASTOS_OBSERVACIONES
                });
            return list;
        }
        catch (SqlException ex)
    ...
    public long COM_GASTOS_EXT_CAB_INSERT(com_gastos_ext_cab cab)
    {
        try
        {
            ComprasDALTableAdapters.QueriesTableAdapter adapter = new ComprasDALTableAdapters.QueriesTableAdapter();
            object id = adapter.H2_COM_GASTOS_EXT_CAB_INSERT(cab.COM_GASTOS_EXT_CAB_ID, cab.COM_GASTOS_EXT_CAB_PACIENTE_ID, Convert.ToDateTime(cab.COM_GASTOS_EXT_CAB_FECHA_SOL),
                Convert.ToDateTime(cab.COM_GASTOS_EXT_CAB_FECHA_AUT), Convert.ToDateTime(cab.COM_GASTOS_EXT_CAB_FECHA_CIR), cab.COM_GASTOS_EXT_CAB_MED_SOL, cab.COM_GASTOS_EXT_CAB_SERV_ID, cab.COM_GASTOS_EXT_CAB_USU_ID, cab.COM_GASTOS_OBSERVACIONES);
            if (id != null) return Convert.ToInt64(id.ToString());
            else return -1;
        }
        catch (SqlException ex)
    ...
    public static void COM_GASTOS_EXT_DET_INSERT(com_gastos_ext_det det, long CabeceraId)
    {
        try
        {
            ComprasDALTableAdapters.QueriesTableAdapter adapter = new ComprasDALTableAdapters.QueriesTableAdapter();
            adapter.H2_COM_GASTOS_EXT_DET_INSERT(CabeceraId, det.COM_GASTOS_EXT_DET_INS_ID, det.COM_GASTOS_EXT_DET_PRV_ID, det.COM_GASTOS_EXT_DET_CANTIDAD,
                det.COM_GASTOS_EXT_DET_PRECIO_PRESU, det.COM_GASTOS_EXT_DET_PRECIO_FACT, det.COM_GASTOS_EXT_DET_USADO, det.COM_GASTOS_EXT_DET_NRO_FACTURA,
                DateTime.Parse(det.COM_GASTOS_EXT_DET_FECHA_FACT));
        }
        catch (SqlException ex)
        {
            throw new Exception(ex.Message);
    ...
    public List<com_gastos_ext_det> Administrar_Insumos_Extras(int tipo, string busqueda, long id)
    {
            ...
            if (tipo == 1)// busqueda
            {
                ComprasDALTableAdapters.H2_Administrar_Insumos_ExtrasTableAdapter adapter = new ComprasDALTableAdapters.H2_Administrar_Insumos_ExtrasTableAdapter();
                ComprasDAL.H2_Administrar_Insumos_ExtrasDataTable aTable = adapter.GetData(tipo, busqueda, id);
                foreach (ComprasDAL.H2_Administrar_Insumos_ExtrasRow row in aTable.Rows)
                {
                    com_gastos_ext_det item = new com_gastos_ext_det();

                    item.COM_GASTOS_EXT_DET_INS_ID = row.COM_INS_GASTOS_EXT_ID;
                    item.COM_INS_GASTOS_EXT_DESC = row.COM_INS_GATOS_EXT_DESC;
                    list.Add(item);
                }
            }
            ...
            {
                ComprasDALTableAdapters.QueriesTableAdapter adapter = new ComprasDALTableAdapters.QueriesTableAdapter();
                adapter.H2_Administrar_Insumos_Extras_ABM(tipo, busqueda, id);
