...
function GetCama(Sala) {
    ...
    $.ajax({
        type: "POST",
        url: "../Json/Hemodinamia.asmx/Lista_Camas",
        data: json,
        contentType: "application/json; charset=utf-8",
        dataType: "json",
...
function GetSala() {
    ...
    $.ajax({
        type: "POST",
        url: "../Json/Hemodinamia.asmx/Lista_Salas_S",
        data: json,
        contentType: "application/json; charset=utf-8",
        dataType: "json",
...
function Protocolo_Guardar() {
    ...
    if (imprimir_comprobate) {
        if ($("#txt_esquema_operatorio").val() == "" && $("#txt_macroscopica").val() == "" && $("#txt_Biopsia").val() == "" && $("#ck_biopsia").is(':checked') == false) {
            alert("No se puede imprimir el protocolo vacío");
            return;
        }   
    }
    ...
    q.Biopsia_Detalle = $("#txt_Biopsia").val();
    q.Cirugia_Id = Id;
    q.Descripcion_Esquema = $("#txt_esquema_operatorio").val();
    q.Descripcion_Macro = $("#txt_macroscopica").val();
    q.Diagnostico_PostOperatorio_Id = diagnosticos_ck;
    q.observaciones = $("#txt_observaciones").val();
    var json = JSON.stringify({ "q": q });
    $.ajax({
        type: "POST",
        url: "../Json/Hemodinamia.asmx/Hemodinamia_Protocolos_Guardar",
        data: json,
        contentType: "application/json; charset=utf-8",
        dataType: "json",
...
function f_Imprimir() {
    $.fancybox({
        ...
        'href': '../Impresiones/Hemodinamia/Hemodinamia_Protocolo.aspx?Id=' + Id,
        'width': '95%',
        'height': '95%',
        'autoScale': false,
        'transitionIn': 'none',
    ...
    if (num > 0) {
        if (imprimir_comprobate) {
            ...
            if (volver_pantalla) {
                ...
                alert("El Protocolo se ha guardado correctamente");
            }
        }
    }
...
function LoadCirugia() {
    ...
    $.ajax({
        type: "POST",
        url: "../Json/Hemodinamia.asmx/Protocolos_Cirugia_Info",
        data: json,
        contentType: "application/json; charset=utf-8",
        dataType: "json",
...
function Cargar_Diagnosticos(Defecto) {
    ...
    $.ajax({
        type: "POST",
        url: "../Json/Hemodinamia.asmx/Diagnostico_Planificar_Hemodinamia",
        data: json,
        contentType: "application/json; charset=utf-8",
        dataType: "json",
...
function Cirugia_Cargada(Resultado) {
    ...
    //$.each(Cirugias, function (index, Cirugia) {
        ...
        DiagnosticoId_Pre = Cirugia.Diagnostico_Id;
...
function LoadProtocolo() {    
    ...
    $.ajax({
        type: "POST",
        url: "../Json/Hemodinamia.asmx/ListProtocolo_ByCirugiaId",
        data: json,
        contentType: "application/json; charset=utf-8",
        dataType: "json",
...
function LoadProtocolo_Cargada(Resultado) {
    ...
    $("#txt_macroscopica").val(qui.Descripcion_Macro);
    if (qui.Biopsia == true)
        $("#ck_biopsia").attr('checked', true);
    $("#txt_Biopsia").val(qui.Biopsia_Detalle);
    u_id = qui.usuario_id;
    $("#txt_observaciones").val(qui.observaciones);    
    PuedoModificarParte();
    $("#span_realizado_por").html(qui.usuario);                    

//    //TENGO QUE HACER QUE TRAIGA EL ANTERIOR.
//    if (qui.Diagnostico_PostOperatorio_Id == null) {
//        diagnosticos_listas_cargadas = DiagnosticoId_Pre;
//        ListaDiagnostico(0, true, Id);        
//        CargarDiagnosticosPre();        
//    }
//    else {
...
function CargarDiagnosticosPre() {    
    var separated = diagnosticos_listas_cargadas.split(",");
    $.each(separated, function (index, los_ids) {
        $("#ck_diag_" + los_ids).prop('checked', true);
    });
...
function PuedoModificarParte() {    
    var json = JSON.stringify({ "U": u_id });
    $.ajax({
        type: "POST",
        data: json,
        url: "../Json/Hemodinamia.asmx/PuedoModificarParte",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (Respuesta) {
            var dato = Respuesta.d;
