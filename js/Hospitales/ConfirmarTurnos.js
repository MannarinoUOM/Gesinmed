...
$(document).ready(function () {
    ...
    if (GET["TLC"] != "" || GET["TLC"] != null || GET["TLC"] != undefined) { TLC = GET["TLC"]; } else { TLC = 0; }
...
function Cargar_Practicas_by_Especialidad(EspecialidadId) {
    ...
    $.ajax({
        ...
        complete: function () {
            ...
            else {// SI ES TELECONSULTA CARGA LA PRACTICA POR DEFAULT
                if (TLC == 1) { $("#txtCodigo").val("450101"); $("#cbo_Practicas").val("450101"); } else {
...
function Cargar_Paciente_Documento_Cargado(Resultado) {
    ...
    $.each(Paciente, function (index, paciente) {
        ...
        //alert('../fotoPerfil/' + paciente.Foto);
        $('#fotopaciente').attr('src', '../fotoPerfil/' + paciente.Foto);
        //$('#fotopaciente').width(100);
...
//function verificaConvenio() { // valida convenio cargado para la practica de teleconsulta

//    if (TLC == 1) {
//        var json = JSON.stringify({ "Seccional": Seccional, "EspecialidadId": EspecialidadId, "Practica": 450101, "Nomenclador": 10 });
//        $.ajax({
//            type: "POST",
//            url: "../Json/ConfirmarTurnos.asmx/verificaConvenio",
//            data: json,
//            contentType: "application/json; charset=utf-8",
//            dataType: "json",
//            success: function (Resultado) { if (Resultado.d == null) { return false; } else { return true; } },
//            error: errores
//        });
//    } else { return true; }
//}
...
$("#btnConfirmarTurno").click(function () {
    // alert(monotributista);
    // if (!verificaConvenio()) { alert("No se valorizo la practica para el convenio. Comuniquese con facturación."); return false; }
    if (pasa == 1) {// doble click
        pasa = 0;
        if (TLC == 1) {
            var json = JSON.stringify({ "Seccional": Seccional, "EspecialidadId": EspecialidadId, "Practica": 450101, "Nomenclador": 10 });
            $.ajax({
                ...
                success: function (Resultado) {
                    if (Resultado.d == null && TLC == 1) { alert("No se valorizo la practica para el convenio. Comuniquese con facturación."); return false; }
                    else {
        ...
        } else {/// para los que no sosn de teleconsulta
        ...
        if (EsEspress != 1 && EsEspress_img != 1) {
            if ($("#btnEmitecoprobante").hasClass("active")) {
                if (TurnoTelefonico == "false") {
                    $.fancybox({
                        ...
                        'onClosed': function () {
                            if (TurnoEmiteBono) {
                                ...
                                window.location.href = "DarTurnos.aspx?ID=" + Actual;
                            }
                    ...
                    parent.location.href = "CargaExpress.aspx";
                    $.fancybox.close();
                }
            }
            else window.location.href = "DarTurnos.aspx?ID=" + Actual;
        }
        else {
