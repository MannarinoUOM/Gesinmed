...
function CargarUltimoEstudio(PacienteId) {
    ...
    $.ajax({
        type: "POST",
        url: "../Json/Internaciones/IntSSC.asmx/AT_INT_EST_ALT_COMP_ULT_BY_PAC",
        data: json,
        contentType: "application/json; charset=utf-8",
        dataType: "json",
...
function CargarData() { //Carga de datos para grabar en BD
    ...
    //obj.medicoid = parent.$("#cbo_Medico").val();
    if (parent.$("#cbo_Medico").val() == undefined) { obj.medicoid = MedicoId; } else { obj.medicoid = parent.$("#cbo_Medico").val(); }
...
function OrdenesEstudiosGuardados(Resultado) {
    var Id = Resultado.d;
    self.location = "../Impresiones/EstudioAltaComplejidad.aspx?Id=" + Id + "&IntId=" + IntId + "&MedicoId=" + MedicoId;
    parent.$("#fancybox-close").show();
}


$("#btnImprimir").click(function () {
    self.location = "../Impresiones/EstudioAltaComplejidad.aspx?Id=" + Protocolo + "&IntId=" + IntId;
    parent.$("#fancybox-close").show();
 });