...
//    document.location = "../Turnos/CargaExpress.aspx" + url;
//}
  ...
  //  alert(Paciente.Fecha_Baja);
    if (!Paciente.Vencido) { //Paciente sin vencimiento
       // alert(Paciente.Discapacidad);
        ...
        $("#txtCelular").val(Paciente.Telefono);
        celCache = Paciente.Telefono;

        $("#txtCelular").attr("disabled", false);
        $("#btnActualizarCel").attr("disabled", false);
        ...
        if (Paciente.Alto_Riesgo == true) { $("#altoRiesgo").show(); } else { $("#altoRiesgo").hide(); }
        if (Paciente.CUIT == 88888888888) { $("#lblMonotributista").css('display', 'inline'); monotributista = 1; } else { $("#lblMonotributista").css('display', 'none'); monotributista = 0; }

        //informa deuda de bonos
        if (parseInt(Paciente.deuda) > 0) { alert("\n DEUDA DEL AFILIADO: $" + parseFloat(Paciente.deuda).toFixed(2).toString()); } 

        //verifica vencimiento recibo sueldo
...
function RecargarPagina(url) {
    document.location = "CargaExpress.aspx" + url;
}
...
function CargarLosTurnos(valor) {
    ...
    Cargar_Turnos($("#cbo_Medico").val(), $("#cbo_Especialidad").val(), $('#txtFecha').val(), $("#txtHora").val(), reservados, libres, $("#cbo_Dias").val(), _sobreturnos, _cancelados,monotributista,TeleConsulta);
}
...
function Cargar_Turnos(MedicoId, EspecialidadId, FechaDesde, HoraDesde, Reservados, Libres, Dias, Sobreturnos, Cancelados, mono, TLC) {
    //alert("despues: " + mono);
    $.ajax({
        type: "POST",
        url: "../Json/DarTurnos.asmx/Turno_List_2",
        data: '{MedicoId: "' + MedicoId + '", EspecialidadId: "' + EspecialidadId + '", sFechaDesde: "' + FechaDesde + '", HoraDesde: "' + '' + '", Reservados: "' + Reservados + '", Libres: "' + Libres + '", Dias: "' + Dias + '", Sobreturnos: "' + Sobreturnos + '", Cancelados: "' + Cancelados + '",monotributista: "' + mono + '", TLC: "' + TLC + '"}',
        contentType: "application/json; charset=utf-8",
        dataType: "json",
...
function Turnos_Cargados(Resultado) {
    ...
    $.each(Turnos, function (index, turnos) {
        ...
        var Adj = "";
        if (turnos.FechaAdj != null) Adj = turnos.FechaAdj + "-" + turnos.Usuario;
...
// CUANDO SELECCIONAN EL CHECK DE UN TURNO DE LA LISTA
function selec(z) {// SEQUITO EL MESAJE QUE PREGUNTA SI EL TURNO SELECCIONADO ES TELECONSULTA YA QUE NO SE HACEN MAS 22/10/2021
//    if (($("#btnOnline").is(":visible") && !$("#btnOnline").prop("checked")) && $("#chk" + z).prop("checked")) {
//        var r = confirm("El turno que desea otorgar es de “Teleconsulta”?.");
//        if (r) {
//            TeleConsulta = 1;
//            Cargar_Medicos_por_Especialidad($("#cbo_Especialidad").val(), 'A', TeleConsulta);
//            CargarLosTurnos(2);
...
$("#btnMulti").click(function () {
    ...
    else { alert("No hay turnos seleccionados."); return; }
});

function ConfirmarTurnos() {
    if (!$.isNumeric($("#afiliadoId").val()) || $("#afiliadoId").val() <= 0) { alert("Paciente no válido."); return false; }
   // alert(TeleConsulta);                                                                                                                                                                                                                                 //PARA TELECONSULTA ELIMINAR SUBIR VERSION
    var url = "CargaExpressDatos.aspx?MedicoID=" + objTurnosSelec[0].MedicoId + "&EspecialidadId=" + objTurnosSelec[0].EspecialidadId + "&Fecha=" + objTurnosSelec[0].Fecha + "&Hora=" + objTurnosSelec[0].Hora + "&Primera=" + $("#cb_pv").is(':checked') + "&TurnoTelefonico=" + $("#cb_ttel").is(':checked') + "&Ids=" + Ids + "&ID="+$("#afiliadoId").val() + "&TLC=" + TeleConsulta + "&mono=" + monotributista;
    Ventana(url);
}
    ...
    $.fancybox(
        {
            ...
            'enableEscapeButton': false,
            'onClosed': function () {
                document.location = "../Turnos/CargaExpress.aspx";
            }
        });
}
...
function Cargar_Medicos_por_Especialidad(Especialidad,Tipo, TLC) {
    var json = JSON.stringify({"Especialidad": Especialidad, "Tipo": Tipo, "TLC": TLC });
    $.ajax({
        type: "POST",
...
function Medicos_por_Especialidad_Cargados(Resultado) {
    ...
    $.each(Medicos, function (index, medicos) {
        //26-10-2015
        //CON ESTO NO APARECEN LOS MEDICOS CUYO NOMBRES SON...
        //TOMOGRAFIA
        //RESONADOR
        //RADIOLOGIA IMG
        //ECOGRAFO I
        //ECOGRAFO II
        //MAMOGRAFIA IMG
        if (medicos.Medico != "RESONADOR" && medicos.Medico != "TOMOGRAFIA" && medicos.Medico != "RADIOLOGIA IMG" && medicos.Medico != "ECOGRAFO I" && medicos.Medico != "ECOGRAFO II" && medicos.Medico != "MAMOGRAFIA IMG")
        $('#cbo_Medico').append(            
...
function Especialidad_Cargadas(Resultado) {
    ...
    $.each(Especialidad, function (index, especialidades) {
        //esto es para que no cargue la especialdiad IMAGENES
        if (especialidades.id != 326) {
            //esto es para que no cargue la especialdiad IMAGENES
...
$(document).ready(function () {
    ...
    $('#cbo_Especialidad').change(function () {
        ...
        $("#TablaTurnos").empty();
        Cargar_Medicos_por_Especialidad($(this).val(), 'A', TeleConsulta);
        $("#btn_Libres").click();
        CargarLosTurnos(2);
        SobreTurnoActivo();
        //TELECONSULTA OCULTAR PARA SUBIR VERSION
        verificarTeleConsultaDisponible($(this).val());
    });
    ...
    Cargar_Medicos_por_Especialidad(0, 'A', TeleConsulta);
    ...
    $("#btn_SobreTurno").click(function () {
        ...
        if ($("#txtHora").val().length == 5) {
            ...
            if (confirm("¿Desea otorgar el Sobreturno para el día " + $("#txtFechaSobre").val() + " a las " + $("#txtHora").val() + "hs. ?"))
...
function AtiendeDiadelaSemanaRespuesta(Respuesta) {
    var r = Respuesta.d;
    if (r) {
        var url = "CargaExpressDatos.aspx?MedicoID=" + $('#cbo_Medico option:selected').val() + "&EspecialidadId=" + $('#cbo_Especialidad option:selected').val() + "&Fecha=" + $("#txtFechaSobre").val() + "&Hora=" + $("#txtHora").val() + "&Primera=" + $("#cb_pv").is(':checked') + '&Actual=' + $("#afiliadoId").val() + "&SobreTurno=true&TurnoTelefonico=" + $("#cb_ttel").is(':checked') + "&Ids=" + objTurnosSelec + "&ID=" + $("#afiliadoId").val() + "&TLC=" + TeleConsulta + "&mono=" + monotributista;
        Ventana(url)
    }
    else {
...
function ForzarLlamadoBox(Box) {
    $.ajax({
        data: '{Box: "' + Box + '"}',
        type: "POST",
        url: "../Json/Turnera/TurneraBonos.asmx/Turnera_Turno_Forzar_Llamar_Bonos",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: PacienteLlamado,
        error: errores
...
function verificarTeleConsultaDisponible(espId) {
    $.ajax({
        data: '{espId: "' + espId + '"}',
        type: "POST",
        url: "../Json/Turnera/TurneraBonos.asmx/verificarTeleConsultaDisponible",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (Resultado) {
            if (Resultado.d == 1) {
                $(".btnOnline").show(); $(".btnOnline").css("display", "inline");
             } else {
                $(".btnOnline").hide();
                TeleConsulta = 0;}  
        },
...
$("#btnOnline").click(function () {
    if ($("#btnOnline").prop('checked') == true) {
        TeleConsulta = 1;
        Cargar_Medicos_por_Especialidad($("#cbo_Especialidad").val(), 'A', TeleConsulta);
        CargarLosTurnos(2);
...
$("#btnActualizarCel").click(function () {
    if ($("#txtCelular").val() != celCache) {
        if ($("#afiliadoId").val() == "") {
            return false;
        } else {
            if ($("#btnActualizarCel").attr("disabled")) return false;
            if (PhoneValidate("", "txtCelular")) {
                $.ajax({
                    type: "POST",
                    url: "../Json/Gente.asmx/ActualizarCelular",
                    data: '{AfiliadoId: "' + $("#afiliadoId").val() + '" , celular: "' + $("#txtCelular").val() + '"}',
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    success: function (Resultado) {
                        if (Resultado.d == 1) {
                            generarLog($("#afiliadoId").val(), $("#txtCelular").val(), "PEDIDO DE TURNO"); celCache = $("#txtCelular").val();
                            alert("Celular Actualizado!.");
                        }
                        else { alert("No se pudo actualizar el Celular!."); }
                    },
                    error: errores
                });
            }
