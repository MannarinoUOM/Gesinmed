...
function List_Mov_by_Insumo_Cargado(Resultado) {
    var Lista = Resultado.d;
    var Encabezado = "<table class='table table-hover table-condensed' style='width: 100%;'><thead><tr><th></th><th>Fecha</th><th>Hora</th><th>Insumo</th><th>Nro. de Lote</th><th style='text-align:right'>Cantidad</th><th></th></tr></thead><tbody>";
    var Contenido = "";
    if (Lista.length == 0) Contenido = "<tr><td></td><td>" + '' + " </td><td> " + '' + " </td><td>" + '<b>***INSUMO SIN MOVIMIENTOS***</b>' + " </td><td> " + '' + " </td><td></td></tr>";
    var Cuenta = 0;
    var i = -1;
    var Invent = "";
    $.each(Lista, function (index, Detalle) {
        if (Detalle.IdInventario == 0) {
            Invent = "<tr id='trinv'><td></td><td>" + '' + " </td><td> " + '' + " </td><td>" + '<b>***INSUMO SIN INVENTARIO***</b>' + " </td><td> " + '' + " </td><td></td></tr>";
            ConInv = false;
        }
        else {
            Invent = "";
            ConInv = true;
        }                                                                                                                             //supuestamente va el lote "+ Detalle. +"
        Contenido = Contenido + "<tr><td></td><td>" + Detalle.Fecha + " </td><td> " + Detalle.Hora + " </td><td>" + Detalle.Insumo + " </td><td>" + Detalle.lote + "</td><td style='text-align:right' > " + Detalle.Cantidad + " </td><td>"+ '' +"</td></tr>";
        Cuenta = Cuenta + Detalle.Cantidad;
        i = i + 1;
        $("#btnPrint").show();
        if (i == Lista.length - 1) Contenido = Contenido + "<tr><td></td><td>" + '' + " </td><td>" + '' + " </td><td> " + '' + " </td><td>" + '<b>Total</b>' + " </td><td style='text-align:right'> " + Cuenta + " </td><td>" + '' + "</td><td></td></tr>"; 
    });
    var Pie = "</tbody></table>";
   ...
   // $("#TablaPedidos_div").attr('text-aling', 'center');
}
...
$("#btnPrint").click(function () {
    ...
    if ($("#chk_Inventario").is(":checked"))
        Inventario = true;
    var Excel = "0";
    if ($("#chkExcel").is(":checked")) Excel = "1";
    $.fancybox(
        {
            'autoDimensions': false,
            'href': "../Impresiones/ImpresionCtaCteInsumo.aspx?InsumoId=" + InsumoId + "&Desde=" + Desde + "&Hasta=" + Hasta + "&Inventario=" + Inventario + "&ConInv=" + ConInv + "&Excel=" + Excel,
            'width': '75%',
            'height': '75%',
            'autoScale': false,
            'transitionIn': 'none',
    ...
    function PrintByRubro() {
     ...
     $.fancybox(
        {
            'autoDimensions': false,
            'href': "../Impresiones/InsumoCtaCteRubro.aspx?RubroId=" + RubroId + "&Desde=" + Desde + "&Hasta=" + Hasta + "&ConInv=" + Inventario + "&Excel=" + Excel,
            'width': '75%',
            'height': '75%',
            'autoScale': false,
            'transitionIn': 'none',
