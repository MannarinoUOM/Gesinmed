...
function List_Medicamento_Medidas_Cargado(Resultado) {
    ...
    $.each(Lista, function (index, Medida) {
        $("#cbo_Presentacion").append($("<option></option>").val(Medida.Id).html(Medida.Medida));
        $("#Ugramaje").append($("<option></option>").val(Medida.Id).html(Medida.Medida));
        if (Medida.Id == Presentacion) $("#cbo_Presentacion").val(Presentacion);
        if (Medida.Id == Unidad) $("#Ugramaje").val(Unidad);
    });
...
function List_Presentacion_Cargado(Resultado) {
    ...
    $.each(Lista, function (index, P) {
        $("#cbo_unidad").append($("<option></option>").val(P.Id).html(P.Presentacion));
        if (P.Id == Presentacion) $("#cbo_unidad").val(Presentacion);
    });
...
$(document).ready(function () {
    ...
    $("#frm_").validate({
        rules: {
            ...
            'gramaje': { required: true, number: true, range: [-99999, 99999] },
            'stockMax': { required: true, number: true, range: [-99999, 99999] },
            'cbo_Rubro': { required: true, number: true, range: [1, 99999] },
            //'cbo_unidad': { required: true, number: true, range: [1, 99999] },
            'precio_fact': { required: true, number: true, range: [-99999, 99999] },
            'Vencimiento_Diasaviso': {required: true,number: true, range: [-99999, 99999] }
        },
        messages: {
            'insumo': { required: '' },
            'stockmin': { required: '', number: '', range: '' },
            'stockMax': { required: '', number: '', range: '' },
            'gramaje': { required: '', number: '', range: '' },
            'cbo_Rubro': { required: '', number: '', range: '' },
            //'cbo_unidad': { required: '', number: '', range: '' },
            'precio_fact': { required: '', number: '', range: '' },
            'Vencimiento_Diasaviso': { required: '' , number: '',range: ''}
        },
    ...
    if (queryObj['id'] != null) {
        ...
        $("#btnEliminar").show();
        $("#btnAgregar").html('<i class=" icon-ok icon-white"></i>&nbsp;Guardar Cambio');
    }
    else {
        List_Rubros();
        List_Presentacion();
...
function Insumos_List_byId_Cargado(Resultado) {
    ...
    Presentacion = ObjInsumo.REM_PRESENTACION;
    Monodroga = ObjInsumo.MONODROGA;
    Laboratorio = ObjInsumo.LAB_ID;
    Rubro = ObjInsumo.REM_RUBRO_ID;

    UGramaje = ObjInsumo.REM_GRAMAJE_ID;
    FormaFarma = ObjInsumo.REM_UNIDADES_ID;

    $("#Presentacion").val(ObjInsumo.REM_PRESENTACION_C.replace(',','.').substring(0,7));
    ...
    $("#GLN").val(ObjInsumo.GLN);
    $("#GTIN").val(ObjInsumo.GTIN);
    $("#Vencimiento_Diasaviso").val(ObjInsumo.Vencimiento_Diasaviso); //numerico
    $("#stockMax").val(ObjInsumo.StockMax); //numerico
...
function Validar() {
    if (parseInt($("#stockmin").val()) >= parseInt($("#stockMax").val())) { alert("El stock minimo debe ser inferior al stock maximo."); return false; }
    if (parseInt($("#stockMax").val()) <= parseInt($("#stockmin").val())) { alert("El stock maximo debe ser superior al stock minimo."); return false; }
    return true;
}

$("#btnAgregar").click(function () {
    var frmValid = $("#frm_").valid();
//    if ($("#Vencimiento_Diasaviso").val() == "") { alert("Ingrese un valor para 'Alertar del vencimiento con'."); return false; }
    if (frmValid) {
...
$("#btnCancelar").click(function () {
 ...
 function LoadData() {
     ...
     if ($("#baja").is(":checked")) ObjInsumo.ELIMINADO = true;
     ...
     ObjInsumo.REM_GRAMAJE = parseFloat($("#gramaje").val().trim()); //Cantidad Gramaje
     ObjInsumo.REM_GRAMAJE_ID = $("#Ugramaje :selected").val(); //Gramaje Presentacion.

//     alert($("#Ugramaje :selected").val());
//     alert($("#cbo_unidad :selected").val());
//     return;
     //ObjInsumo.REM_PRESENTACION_C = $("#Presentacion").val().replace('.', ','); //Presentacion Cantidad(Tamaño).
     //ObjInsumo.REM_PRESENTACION_ID = $("#cbo_Presentacion :selected").val(); //Combo Presentacion

     ObjInsumo.REM_PRESENTACION_ID = $("#cbo_unidad :selected").val(); 

     //ObjInsumo.REM_UNIDADES_ID = $("#cbo_unidad :selected").val(); // Forma Farma

     ObjInsumo.REM_UNIDADES_ID = $("#Ugramaje :selected").val(); 
     
     ObjInsumo.REM_RUBRO_ID = $("#cbo_Rubro :selected").val();
     ObjInsumo.REM_PRECIO = $("#precio_fact").val();
     ObjInsumo.REM_PRESENTACION_C = 0;
     ...
     ObjInsumo.CANT_BLISTER = 1;
     ObjInsumo.LAB_ID = $("#cbo_laboratorio :selected").val();

     ObjInsumo.GLN = $("#GLN").val().trim().toUpperCase();
     ObjInsumo.GTIN = $("#GTIN").val().trim().toUpperCase();
     ObjInsumo.Vencimiento_Diasaviso = $("#Vencimiento_Diasaviso").val().trim(); //numerico
     ObjInsumo.StockMax = $("#stockMax").val();
