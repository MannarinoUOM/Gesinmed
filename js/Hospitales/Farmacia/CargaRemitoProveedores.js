...
function Get_Insumo_by_Id_Cargado(Resultado) {
    ...
    //if (parseInt(Insumo.StockMax) < parseInt(Insumo.STO_CANTIDAD) && Nombre_Alert.length > 0) { alert("El insumo (" + Nombre_Alert + ") está por encima del stock máximo."); Nombre_Alert = ""; }
    $("#cbo_Medicamento").val($("#txt_Medicamento").val());
    $("#txtPrecioVenta").val(Insumo.REM_PRECIO);
}
...
function ConCeros8(Cifras) {
 ...
 function ValidarPrecios() {
     if (parseFloat($("#txtPrecioVenta").val()) < parseFloat($("#txtPrecioCompra").val())) { alert("El precio de venta es menor al precio de compra, para continuar cargando el remito ingrese $0.\nNo olvide corregir los precios."); return false; }
     return true;
 }
 ...
 $("#btnAgregarMedicamento").click(function () {
     ...
     if (valid && $("#Medicamento_val").html() != '0') {
         ...
         objMedicamento.Precio_Compra = 0; //($("#txtPrecioCompra").val()).replace(",", ".");
         objMedicamento.Estado = Estado;
         objMedicamento.Nombre = Nombre;
         objMedicamento.Precio_Venta = $("#txtPrecioVenta").val();
...
function Editar(Nro) {
    ...
    $("#txtPrecioVenta").val(objMedicamentos[Nro].Precio_Venta);
...
function RenderizarTabla() {
    var Encabezado = "<table class='table table-hover table-condensed' style='width: 100%;'><thead><tr><th></th><th>Insumo</th><th>Cantidad de Unidades</th><th>Nro Lote</th><th style='display:none;'>Compra (x unidad)</th><th>Venta (x unidad)</th><th>Vencimiento</th></tr></thead><tbody>";
    var Contenido = "";
    ...
    for (var i = 0; i <= Total; i++) {
        ...
        if (objMedicamentos[i].Estado == 1) {
            if (remitoId > 0) {
                Contenido = Contenido + "<tr><td><a id='Editar" + i + "' onclick='Editar(" + i + ");' class='btn btn-mini' title='Editar Medicamento'><i class='icon-edit'></i></a><a id='Elminar" + i + "'onclick='Eliminar(" + i + ");' style='display:none;' class='btn btn-mini btn-danger' rel='tooltip' title='Quitar Medicamento'><i class='icon-remove-circle icon-white'></i></a></td><td> " + objMedicamentos[i].Nombre + "(" + objMedicamentos[i].Insumo_Id + ") </td><td> " + objMedicamentos[i].Cantidad + " </td><td>" + objMedicamentos[i].NroLote + " </td><td style='display:none;'> $ " + objMedicamentos[i].Precio_Compra + " </td><td> $ " + objMedicamentos[i].Precio_Venta + " </td><td>" + objMedicamentos[i].FechaVencimiento + " </td></tr>";
            }
            else
