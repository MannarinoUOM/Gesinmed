...
$("#btnCargar").click(function () {
    window.location = "DevolucionporPrestamo.aspx";
});
...
function Buscar_Pedidos() {
    ...
    $.ajax({
        type: "POST",
        url: "../Json/Farmacia/Farmacia.asmx/BuscarDevolucionesporPrestamo",
        data: json,
...
function Pedidos_Cargados(Resultado) {
    ...
    if (Pedidos.length > 0) {
        var Tabla_Datos = "";
        Tabla_Titulo = "<table id='TablaPedidos_div' class='table table-hover table-condensed' style='width: 100%;'><thead><tr><th>Nro Ingreso</th><th>Detalle</th><th>Fecha</th><th>Usuario</th></tr></thead><tbody>";
        $.each(Pedidos, function (index, Pedido) {
            Tabla_Datos = Tabla_Datos + "<tr";
            Tabla_Datos = Tabla_Datos + " style='cursor:pointer' onclick=Ventana('DevolucionporPrestamo.aspx?Id=" + Pedido.Pedido_Id + "');";
        ...
        var leyenda = "No hay ingresos existentes.";
        if ($("#txtNroPed").val().trim().length > 0) leyenda = "No existe el nro. de ingreso " + $("#txtNroPed").val().trim();
