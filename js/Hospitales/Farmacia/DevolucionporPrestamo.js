    ...
    $.fancybox(
        {
            ...
            'href': '../Impresiones/DevolucionporPrestamo_Print.aspx?DevolucionId=' + Pedido_Id,
            'width': '75%',
            'height': '75%',
            'autoScale': false,
            'transitionIn': 'none',
...
$("#cbo_Medicamento").typeahead({
    ...
    updater: function (selection) {
        ...
        traeLoteInsumo(mapped[selection]);// cargar combo lote
    },
    minLength: 4,
    items: 10
});
...
function LoadPedido() {
    $.ajax({
        type: "POST",
        url: "../Json/Farmacia/Farmacia.asmx/BUSCAR_FAR_DEVOLUCION_PRESTAMO_CAB_BY_ID",
        data: '{Id: "' + Pedido_Id + '"}',
        contentType: "application/json; charset=utf-8",
        dataType: "json",
...
function LoadPedido_Cargado(Resultado) {
    ...
    $("#txtDevuelve").html(PedidoCab.Servicio);
...
function LoadDetalles() {
    ...
    $.ajax({
        type: "POST",
        url: "../Json/Farmacia/Farmacia.asmx/BUSCAR_FAR_DEVOLUCION_PRESTAMO_DET_BY_ID",
        data: '{Id: "' + Pedido_Id + '"}',
        contentType: "application/json; charset=utf-8",
        dataType: "json",
...
function LoadPedidoDet_Cargado(R) {
    ...
    $.each(Detalles, function (index, Detalle) {
        ...
        //carga el lote con el que se genero
        objMedicamento.LOTE = Detalle.LOTE;
        //alert(objMedicamento.LOTE);
...
$("#btnAgregarMedicamento").click(function () {
    ...
    if (valid && $("#Medicamento_val").html() != '0') {
        ...
        //se agrega el lote
        objMedicamento.lote = $("#cboLote :selected").val();
...
function Editar(Nro) {
    ...
    traeLoteInsumo(objMedicamentos[Nro].Insumo_Id);
    //alert(objMedicamentos[Nro].LOTE);
    $("#cboLote").val(objMedicamentos[Nro].LOTE);
}
...
function Delete_Detalles() {
    ...
    $.ajax({
        data: json,
        url: "../Json/Farmacia/Farmacia.asmx/FAR_DEVOLUCION_PRESTAMO_DET_DELETE",
        type: "POST",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
...
function Insert_Detalles() {
    ...
    $.ajax({
        data: json,
        url: "../Json/Farmacia/Farmacia.asmx/INSERT_FAR_DEVOLUCION_PRESTAMO_DET",
        type: "POST",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
...
function Insert_Pedido() {
    var f = {};
    f.Servicio_Nombre = $("#txtDevuelve").val().trim().toUpperCase();
    f.Pedido_Id = Pedido_Id;
    var json = JSON.stringify({ "f": f });
    $.ajax({
        data: json,
        url: "../Json/Farmacia/Farmacia.asmx/Insert_DevolucionPrestamo_Cab",
        type: "POST",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
...
function Insert_PPS_Det_Cargado(Resultado) {
    ...
    $.fancybox(
        {
            'autoDimensions': false,
            'href': '../Impresiones/DevolucionporPrestamo_Print.aspx?DevolucionId=' + Id,
            'width': '75%',
            'height': '75%',
            'autoScale': false,
            'transitionIn': 'none',
            ...
            'enableEscapeButton': false,
            'onClosed': function () {
                window.location.href = "DevolucionporPrestamo.aspx";
            },
            'preload': true,
            'onComplete': function f() {
                jQuery.fancybox.showActivity();
...
$("#btnPedidos").click(function () {
    window.location = "DevolucionporPrestamo_Buscar.aspx";
});
