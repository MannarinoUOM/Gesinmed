...
function CargarPedidosCAB_by_EXPID(EXP_ID) {
    ...
    $.ajax({
        ...
        success: function (Resultado) {
            ...
            Tabla_Titulo = "<table id='TablaPedidos' class='table table-condensed'><thead style='font-weight:Bold'><tr style='font-weight:Bold; font-size:larger'>" +
            "<th style='font-weight:Bold'><b>Nro. Ped.</b></th><th><b>Ingreso</b></th><th><b>Receta</b></th><th><b>Usuario</b></th></tr></thead><tbody>";
            $.each(cabeceras, function (index, exp) {
                var class_row = "";
                var leyenda = "";
                //alert(exp.EXP_PED_EDITABLE);
                if (!exp.EXP_PED_EDITABLE) {
                    class_row = "warning ";
                    leyenda = "<b>PEDIDO ENTREGADO</b>";
                }
                if (exp.EXP_PED_EDITABLE) {
                    Tabla_Datos += "<tr class='" + class_row + "tr_CAB' id='tr" + exp.EXP_PED_ID + "' data-placement='bottom' title='" + leyenda + "' onclick='AbrirPedidoCab(" + exp.EXP_PED_ID + ")'><td>" + exp.EXP_PED_ID + "</td><td id='EXP_PED_FECHA" + exp.EXP_PED_ID + "'>" + exp.EXP_PED_FECHA + "</td><td>" + exp.EXP_PED_FECHA_RECETA + "</td>" +
                    //"<td id='EXP_PED_DURACION" + exp.EXP_PED_ID + "'>" + exp.EXP_PED_DURACION + "</td>" +
                    "<td>" + exp.EXP_PED_USU_NOM + "</td><td id='EXP_PED_OBS" + exp.EXP_PED_ID + "' style='display:none;'>" + exp.EXP_PED_OBS + "</td><td style='display:none;' id='EXP_PED_URGENTE" + exp.EXP_PED_ID + "'>" + exp.EXP_PED_URGENTE + "</th><td style='display:none;' id='EXP_PED_FECHA_RECETA" + exp.EXP_PED_ID + "'>" + exp.EXP_PED_FECHA_RECETA + "</th><td id='EXP_ES_60_90DIAS" + exp.EXP_PED_ID + "' style='display:none;'>" + exp.EXP_PED_ES_60_90 + "</td></tr>";
                } else {
                //<p style='font-weight:bold;'>(" + exp.EXP_PED_ID_ORIGEN + ")</p>
                    Tabla_Datos += "<tr class='" + class_row + "tr_CAB' id='tr" + exp.EXP_PED_ID + "' data-placement='bottom' title='" + leyenda + "'><td style='cursor:default'>" + exp.EXP_PED_ID + "</td>" +
                    "<td id='EXP_PED_FECHA" + exp.EXP_PED_ID + "'  style='cursor:default'>" + exp.EXP_PED_FECHA + "</td>" +
                    "<td style='cursor:default'>" + exp.EXP_PED_FECHA_RECETA + "</td>" +
                    "<td style='cursor:default'>" + exp.EXP_PED_USU_NOM + "</td>" +
                    "<td id='EXP_PED_OBS" + exp.EXP_PED_ID + "' style='display:none;'>" + exp.EXP_PED_OBS + "</td><td style='display:none;' id='EXP_PED_URGENTE" + exp.EXP_PED_ID + "'>" + exp.EXP_PED_URGENTE + "</th><td style='display:none;' id='EXP_PED_FECHA_RECETA" + exp.EXP_PED_ID + "'>" + exp.EXP_PED_FECHA_RECETA + "</th><td id='EXP_ES_60_90DIAS" + exp.EXP_PED_ID + "' style='display:none;'>" + exp.EXP_PED_ES_60_90 + "</td></tr>";
                }
            });
...
function CargarDetallesPED(PDT_PED_ID) {
    ...
    $.ajax({
        type: "POST",
        data: json,
        url: "../Json/Compras/ComprasInternacion.asmx/EXP_PEDIDOS_ENTREGAS_DET_BY_PED_ID",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        beforeSend: function () {
            $("#btnPrint100").hide();
        ...
        success: function (Resultado) {
            ...
            var Tabla_Datos = "";
            var Tabla_Fin = "";
            Tabla_Titulo = "<table id='TablaPedidoDetalles' class='table table-hover table-condensed' style='font-size:11px;'>" +
            "<thead style='font-size:larger'><tr><th>Insumo</th><th>Cantidad Pedida</th><th>Fecha Entrega</th><th>Cantidad Entregada</th><th>Saldo</th><th>Usuario</th><th>Imprimir</th></tr></thead><tbody>";
            $.each(cabeceras, function (index, exp) {
                var color = "";
                var cursor = "";
                if (exp.PDT_SALDO == 0) { color = "yellow"; cursor = "default"; }

                Tabla_Datos += "<tr style='background-color:" + color + "'  class='tr_DET' id='tr" + exp.PDT_ID + "' onclick='MostrarDetalle(" + exp.PDT_ID + ")'>" +
                "<td style='display:none;' id='ROWINDEX_" + index + "'>" + exp.PDT_ID + "</td>" +
                "<td style='display:none;' id='PDT_ID" + exp.PDT_ID + "'>" + exp.PDT_ID + "</td>" +
                "<td style='display:none;' id='PEE_NUMERO_REM" + exp.PDT_ID + "'>" + exp.PEE_NUMERO_REM + "</td>" +

                //"<td id='PDT_INS_NOM" + exp.PDT_ID + "'>" + exp.PDT_INS_NOM + "</td>" +

                "<td style='cursor:" + cursor + "' onclick='VerDocumentos(" + exp.PDT_ID + ")'><a class='btn btn-mini'>Ver documentos</></td>" + // boton insumo
                "<td style='cursor:" + cursor + "' id='PDT_CANTIDAD" + exp.PDT_ID + "'>" + exp.PDT_CANTIDAD + "</td>" +
                // "<td id='PDT_POR_DESC" + exp.PDT_ID + "'>" + exp.PDT_POR_DESC + "</td>" +
                "<td style='cursor:" + cursor + "' id='PEE_FEC_ENTREGA" + exp.PDT_ID + "'>" + MostrarFecha(exp.PEE_FEC_ENTREGA) + "</td>" +
                "<td style='cursor:" + cursor + "' id='PEE_CANT_ENTR" + exp.PDT_ID + "'>" + exp.PEE_CANT_ENTR + "</th>" +
                "<td style='cursor:" + cursor + "' id='PDT_SALDO" + exp.PDT_ID + "'>" + exp.PDT_SALDO + "</td>" +
                //"<td id='DEPOSITO_DESC" + exp.PDT_ID + "'>" + exp.DEPOSITO_DESC + "</th>" +
                "<td style='display:none;cursor:" + cursor + "' id='PEE_PRE_UNI" + exp.PDT_ID + "'>" + exp.PEE_PRE_UNI + "</th><td style='display:none;' id='INS_ULT_PRECIO" + exp.PDT_ID + "'>" + exp.INS_ULT_PRECIO + "</th><td id='PDT_USUARIO_NOM" + exp.PDT_ID + "'>" + exp.PDT_USUARIO_NOM + "</th><td style='display:none;' id='PDT_OBS" + exp.PDT_ID + "'>" + exp.PDT_OBS + "</th><td style='display:none;' id='PDT_INS_ID" + exp.PDT_ID + "'>" + exp.PDT_INS_ID + "</th><td style='display:none;' id='PEE_DEP_ID" + exp.PDT_ID + "'>" + exp.PEE_DEP_ID + "</th><td style='display:none;' id='PEE_ID" + exp.PDT_ID + "'>" + exp.PEE_ID + "</th><td style='display:none;' id='USU_MED" + exp.PDT_ID + "'>" + exp.USU_MED + "</td>" +
                "<td style='cursor:" + cursor + "'><input type='checkbox' class='Imprimir' id='detImprimir" + exp.PDT_ID + "'/></td>" +
                "<td id='Nremito" + exp.PDT_ID + "' style='display:none' >" + exp.Nremito + "</td></tr>";
                //"<td><input type='checkbox'/></td></tr>";
                //G_PEE_NUMERO_REM = exp.Nremito;
                G_PEE_NUMERO_REM = exp.PEE_NUMERO_REM;
            });
            Tabla_Fin = "</tbody></table>";
...
$("#btnGuardarDET").click(function () {
    ...
    $.ajax({
        type: "POST",
        data: json,
        url: "../Json/Compras/ComprasInternacion.asmx/EXP_PEDIDOS_ENTREGAS_DET_INSERT",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (Resultado) {
            if (Resultado.d > 0) {
...
$("#btnPedidosVolver").click(function () {
    if (G_ExpId > 0)
        window.location = "Compras_Expediente_Pedidos_Internacion.aspx?ExpId=" + G_ExpId;
});
        ...
        success: function (Resultado) {
            ...
            if (ent_cab != null) {
                ...
                if (ent_cab.PEE_IMPRESO) {
                    G_Impreso = true;
                    $("#btnPrint").show(); //La entrega ya fue impresa...
                    $("#btnPrint100").hide();
                }
                else {
                    G_Impreso = false;
...
$("#btnPrint").click(function () {
    listaImpresion = "";
    $(".Imprimir").each(function (index, item) {
        if ($(this).is(':checked')) {
            listaImpresion = listaImpresion + $(this).attr('id').replace("detImprimir", "") + ",";
            //alert(listaImpresion);
        }
    });

    // $.each(listaImpresion, function (index, item) { alert(item); });
    //return false;
    
    // alert(G_PEE_NUMERO_REM);
    if (listaImpresion.length <= 0) { alert("Seleccione algún detalle para imprimir."); return false; } else {
        //if (G_PEE_NUMERO_REM > 0)
        Imprimir("../Impresiones/Compras/Compras_ImpresionEntrega_Internacion.aspx?ENT_CAB_ID=" + G_PEE_NUMERO_REM + "&Duracion=" + G_DURACION + "&PED_CAB_ID=" + G_PedCAB + "&ENT_IDS_DETALLES=" + listaImpresion + "&PDT_PED_ID=" + G_PedCAB);
...
function Print_AnalizarDescuentos() {
    ...
    $('.tr_DET').each(function (index) {
        ...
        if ($("#PEE_CANT_ENTR" + PDT_ID).html() > 0) //Si el insumo posee entregas
        {
            ...
            if ($("#PDT_POR_DESC" + PDT_ID).html() == 100) { $("#btnPrint100").hide(); } //Habilito impresion al 100%
        }
    });
}
