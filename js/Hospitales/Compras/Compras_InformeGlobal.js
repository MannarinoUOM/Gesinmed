...
Paciente,Seccional,Deposito) {
    ...
    $.ajax({
        ...
        success: function (Resultado) {
            ...
            if (Pedidos.length > 0) { listar = 1; } else { listar = 0; }
...
$("#btnExcel").click(function () {
    if (listar == 1) {
        ...
        if ($("#txtFechaDesde").val().length == 0) FechaDesde = '01/01/1900'; else FechaDesde = $("#txtFechaDesde").val();
        if ($("#txtFechaHasta").val().length == 0) FechaHasta = '01/01/1900'; else FechaHasta = $("#txtFechaHasta").val();
        if ($("#txtNroExpDesde").val().length == 0) Nro_Remito_Desde = 0; else Nro_Remito_Desde = $("#txtNroExpDesde").val();
        if ($("#txtNroExpHasta").val().length == 0) Nro_Remito_Hasta = 0; else Nro_Remito_Hasta = $("#txtNroExpHasta").val();
        if ($("#txtNroPedidoDesde").val().length == 0) NroPedido_Desde = 0; else NroPedido_Desde = $("#txtNroPedidoDesde").val();
        if ($("#txtNroPedidoHasta").val().length == 0) NroPedido_Hasta = 0; else NroPedido_Hasta = $("#txtNroPedidoHasta").val();


        $.fancybox({
            'autoDimensions': false,
            'href': "../Impresiones/Compras/ExcelControlEntregasCopmrspas.aspx?FechaRemito_Desde=" + FechaDesde + "&FechaRemito_Hasta=" + FechaHasta + "&Nro_Remito_Desde=" + Nro_Remito_Desde + "&Nro_Remito_Hasta=" +
             Nro_Remito_Hasta + "&Nom_Insumo=" + $("#medicamento").val().trim() + "&NroPedido_Desde=" + NroPedido_Desde + "&NroPedido_Hasta=" + NroPedido_Hasta + "&Pendientes=" + $("#chk_Pendientes").is(":checked") +
            "&Entregados=" + $("#chk_Entregados").is(":checked") + "&Paciente=" + $("#Paciente").val().trim() + "&Seccional=" + $("#cbo_Seccional :selected").val() + "&Deposito=" + 0,
            'width': '75%',
            'height': '75%',
            'autoScale': false,
            'transitionIn': 'none',
